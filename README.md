...
现在鼠标离开组件超过1s，组件也不会自动隐藏。
 - 在设置 - 调试里增加一个选项，控制鼠标离开组件后是否自动隐藏。
F11 全屏，F11 退出全屏。全屏模式下，按钮显示为 enable 颜色。全屏模式下，双击标题栏也可以取消全屏，并取消窗口最大化。
触屏优化
图片旋转还有 bug
...
[] 缩略图打成 bundle，（例如 tar，你有更好的打包策略吗？），包大小可设置，默认为 64MB，可选 4-256MB。同一个目录里的图片尽量放在一个包里，如果超过大小限制了再放在新的包里。
