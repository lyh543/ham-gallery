...
WinUI 3 默认没有“滚动到头拉扯弹回去”的弹性动画（iOS 那种 overscroll bounce）。

详细说明

• ListView / GridView / ScrollViewer（旧版）：滚动到顶/底后直接卡死，没有回弹。

• ScrollView（新版 WinUI3 控件）：同样默认无回弹。

• 触摸 vs 鼠标：触摸滑动到边界也不会弹，只是停住；鼠标滚轮更不会。

如果你想要回弹（iOS 风格）

WinUI 3 没有内置开关，必须自己写，通常用 Composition API + InteractionTracker 实现。

一句话结论

WinUI 列表默认没有回弹动画，要做必须自己实现。
...
[] 【排序问题】设置全局的排序顺序和每个相册的排序顺序，在 UI 上添加图标，添加打勾。
