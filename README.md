...
关于缩略图：
[] 修改目录以后，需要验证 Album 里的文件是否还在，不在的删除掉。对于新增的目录也需要进行扫描，但不需要主动计算缩略图。
[] 设置页面，添加一个打开缩略图存放目录的选项
[] 扫描的时候，加载不出来图片了
[] 在设置界面扫描的时候，也需要准实时更新缩略图缓存的总大小。在数据库存储缩略图的大小，这样就可以从数据库查询缩略图的总大小了。
[] 在设置界面主动扫描的时候，可以使用全部的 CPU 资源。
