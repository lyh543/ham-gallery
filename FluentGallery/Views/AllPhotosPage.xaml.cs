...
public sealed partial class AllPhotosPage : Page
{
    ...
    private async void Delete_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (!await ConfirmDeleteAsync(selected)) return;
    ...
    {
        if (ViewModel.AllPhotoItems.Count > 0)
        ...
        bool isMove)
    {
        ...
        var targetName = Path.GetFileName(targetDir.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar));
        if (!await ConfirmDirectoryActionAsync(items, targetName, isMove)) return;
    ...
    private async Task<bool> ConfirmDeleteAsync(IReadOnlyList<PhotoItemViewModel> items)
    {
